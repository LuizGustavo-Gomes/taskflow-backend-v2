...
import TaskModel, { NewTask, UpdateTask, Task } from '../models/taskModel.js';

// Isso é um placeholder para quando você tiver autenticação de verdade.
interface AuthenticatedRequest extends Request {
    userId?: number; // Propriedade adicionada por um middleware de autenticação
}
...
const taskController = {
    ...
    async createTask(req: AuthenticatedRequest, res: Response): Promise<void> {
        ...
        const newTaskData: NewTask = { title, description, user_id };
    ...
    async getTasksByUserId(req: AuthenticatedRequest, res: Response): Promise<void> {
        // Por enquanto, vamos obter o user_id dos parâmetros ou query para testes.
        // **IMPORTANTE:** No futuro, remova isso e use req.userId do JWT!
        const userId = parseInt(req.params.userId || req.query.userId as string); // user_id temporariamente via param/query
