...
const userController = {
    async register(req: Request<{}, {}, RegisterRequestBody>, res: Response): Promise<void> {
        ...
        try {
            ...
            const existingUserByEmail: User | undefined = await UserModel.findByEmail(email);
            ...
            const existingUserByUsername: User | undefined = await UserModel.findByUsername(username);
            ...
            const newUser: User = await UserModel.create(username, email, hashedPassword); 
        ...
        } catch (error: any) { // Tipagem 'any' permite acessar 'code' e 'constraint'
